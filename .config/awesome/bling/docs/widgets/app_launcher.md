...
local args = {
    ...
    favorites = { "brave-browser-nightly", "wezterm" }                -- Favorites are given priority and are bubbled to top of the list
    search_commands = true                                            -- Search by app name AND commandline command
    skip_names = { "Discord" }                                        -- List of apps to omit from launcher
    skip_commands = { "pcmanfm" }                                      -- List of commandline commands to omit from launcher
